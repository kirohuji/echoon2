# guideready MVP 前端实现方案（shadcn + base-ui + 主题化 + 国际化）

本文面向 `guideready` 的 MVP 前端落地，参考：

- `lingoPath/apps/frontend/components.json`（shadcn 配置风格与别名）
- `lingoPath/apps/frontend/package.json`（依赖选型）
- `guideready/docs/导游说-页面布局文档.md`（页面信息架构与区域布局，**必须严格对齐**）
- `guideready/docs/导游说-需求文档.md`（功能需求与业务规则，**必须严格对齐**）
...
`guideready` 的页面布局必须严格遵循 `导游说-页面布局文档.md`，本方案只负责技术落地，不改变原布局定义。
...
`guideready` 的 MVP 功能范围与交互规则必须严格遵循 `导游说-需求文档.md`，不得自行删减核心能力。
...
6. 接入请求层与最小业务流
7. 全量替换硬编码文案为 `t()` 调用
8. 逐页对照 `导游说-页面布局文档.md` 做布局验收（通过后才允许进入联调）
9. 逐条对照 `导游说-需求文档.md` 做功能验收（通过后才允许提测）

---

## 12. 备注：为什么这套组合适合 guideready MVP
