...
GuideReady 是一个面向导游资格证口语考试的**全栈训练系统**。系统模拟真实考试场景，提供题库浏览、口语练习、模拟考试、AI 发音反馈、TTS 语音合成等核心功能，帮助考生高效备考。
...
| 多端支持 | Web SPA + iOS 原生 (Capacitor) |

### 1.3 关键指标

- **API 模块数**：11 个 NestJS 业务模块
- **数据库模型数**：21 个 Prisma 模型 + 3 个枚举
- **前端路由数**：6 个页面路由
- **TTS 提供商**：2 个（MiniMax、Cartesia）
- **外部服务数**：5 个（腾讯云 COS、DeepSeek、MiniMax、Cartesia、Whisper）
...
├── apps/
│   ├── backend/                     # @guideready/backend — NestJS 后端
│   │   ├── prisma/
...
│   │   ├── tsconfig.json
│   │   └── nest-cli.json
│   └── frontend/                    # @guideready/frontend — React 前端
...
| 包名 | 路径 | 入口 |
|---|---|---|
| `@guideready/backend` | `apps/backend` | `node dist/main.js` |
| `@guideready/frontend` | `apps/frontend` | Vite 构建 → Nginx 托管 |
...
```json
{
  ...
  "dev:backend": "pnpm --filter @guideready/backend dev",
  "dev:frontend": "pnpm --filter @guideready/frontend dev",
  "build": "pnpm -r build",
  "lint": "pnpm -r lint",
...
| `preferences.store` | 用户偏好 | localStorage |
...
request.interceptors.request.use((config) => {
  const token = localStorage.getItem('guideready-bearer-token');
...
request.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      localStorage.removeItem('guideready-bearer-token');
...
|---|---|---|
| `#/` | 题库首页 (HomePage) | 需要 |
| `#/practice/:topicId` | 练习页 (PracticePage) | 需要 |
| `#/mock` | 模考页 (MockPage) | 需要 |
| `#/profile` | 个人中心 (ProfilePage) | 需要 |
| `#/member` | 会员页 (MemberPage) | 需要 |
| `#/auth/login` | 登录页 (LoginPage) | 不需要 |
| `#/auth/register` | 注册页 (RegisterPage) | 不需要 |
...
RUN pnpm install --frozen-lockfile --filter @guideready/backend
COPY apps/backend/ ./apps/backend/
RUN pnpm --filter @guideready/backend exec prisma generate
...
RUN pnpm install --frozen-lockfile --prod --filter @guideready/backend
COPY --from=builder /app/apps/backend/dist ./apps/backend/dist
COPY --from=builder /app/apps/backend/prisma/schema.prisma ./apps/backend/prisma/
RUN pnpm --filter @guideready/backend exec prisma generate
EXPOSE 3001
CMD ["pnpm", "--filter", "@guideready/backend", "start:prod"]
```
...
RUN pnpm install --frozen-lockfile --filter @guideready/frontend
...
# 重置数据库（开发环境）
docker compose exec backend pnpm --filter @guideready/backend prisma:seed
```
...
| 文档 | 说明 |
|---|---|
| [导游说-需求文档.md](./导游说-需求文档.md) | 产品需求说明 |
| [导游说-页面布局文档.md](./导游说-页面布局文档.md) | 页面布局与交互设计 |
| [导游说-后端与数据库实现文档.md](./导游说-后端与数据库实现文档.md) | 后端 API 与数据库详细设计 |
| [guideready-MVP前端实现方案.md](./guideready-MVP前端实现方案（shadcn+base-ui+主题化+国际化）.md) | 前端技术方案 |
